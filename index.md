...
More details in Chapter \@ref(packageci). 
...
In some cases the `yaml` will need to be modified. More detail on understanding the `yaml` files can be found in Chapter \@ref(understandingyaml). 

## Extensions

We experimented with seting up continuous integration with a reproducible environment using `renv`, which you can read about in Chapter \@ref(testingrenv).
